...
const ResultV1 = () => {
  ...
  function inds(arr) {
    ...
    function indsHelper(myq, n) {
      let ans_i = myq.indexOf(n);
    ...
    for (let i = 0; i < arr.length; i++) {
      const item = arr[i];
      let iam_q = true;
      if (item.sdflk24234.length > 0) {
        for (let iq of item.sdflk24234) {
          for (let ans of iq.answers) {
            if (ans.ansucc === true) {
              if (iam_q === true) {
                qlen = qlen + item.sdflk24234.length;
                iam_q = false;
              }
              indsHelper(iq.answers, ans);
            }
          }
        }
      }
